  ...
  ipcMain.handle('db:seed', () => {
    ...
    try {
      ...
      for (const f of factories) {
        ...
        const r = dbService.execute('SELECT id FROM factory WHERE name = ?', [f.name])
      ...
      for (let fi = 0; fi < factories.length; fi++) {
        for (let pi = 0; pi < periods.length; pi++) {
          ...
          dbService.execute(insertSql, [periods[pi], factoryIds[fi], ...values])
        }
      }
      ...
      const dashboardDefs = [
        ...
        },
        {
          ...
          analysis: '庫存周轉天數整體維持穩定，但模具類庫存周轉天數偏高（85天以上），佔用大量資金。原材料周轉約 45 天、在制品 18 天、成品 22 天，基本處於行業合理水準。各廠區間的庫存管理水平差異不大，但仍有優化空間。',
          actions: '1. 模具庫存周轉天數過高，需評估閒置模具的處理方案（報廢、轉售或再利用）\n2. 推動 JIT（即時生產）模式，目標將原材料周轉天數降至 35 天\n3. 建立庫存異常預警機制，當單一品類周轉天數超過標準 1.5 倍時自動通知\n4. 每季進行庫存盤點與呆滯分析，呆滯超過 180 天的庫存需提出處理方案',
        },
        {
          name: '報酬率追蹤', description: 'ROE/ROA 與利潤走勢', sort_order: 4,
          analysis: 'ROE 與 ROA 均呈穩步上升趨勢，顯示資本運用效率持續改善。ROE 維持在 7-9% 區間，ROA 約在 3.5-4.5% 區間。獲利率方面，毛利率穩定在 19-21%，但營業利潤率有微幅下降趨勢，銷管研費用增長速度需要關注。',
          actions: '1. 設定 ROE 年度目標 10%，制定對應的利潤增長與資本效率計畫\n2. 檢討銷管研費用結構，識別可優化項目，目標將費用率控制在 8% 以內\n3. 評估股利政策與再投資比例，在股東回報與成長投資間取得平衡\n4. 每季向董事會報告資本報酬率變化，並與同業 benchmark 比較',
        },
      ]
      for (const d of dashboardDefs) {
        dbService.execute(
          `INSERT OR IGNORE INTO dashboard (name, description, sort_order, analysis, actions) VALUES (?, ?, ?, ?, ?)`,
